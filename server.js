...
/* ───────────────────────── database (SQLite, with JSON-file fallback) ─────────────────────────
   Every Telegram user is one row (key = Telegram ID), so accounts/balances are separate and permanent.
   The whole data set is also kept in memory for speed and written to disk shortly after each change. */
...
const DB_FILE = path.join(DATA_DIR, 'db.json');                 // legacy / fallback file
const SQLITE_FILE = path.join(DATA_DIR, 'cashflow.sqlite');
fs.mkdirSync(DATA_DIR, { recursive: true });

let sql = null, ENGINE = 'json';
if (env('DB_ENGINE', 'sqlite') !== 'json') {
  try {
    const { DatabaseSync } = require('node:sqlite');
    sql = new DatabaseSync(SQLITE_FILE);
    sql.exec('PRAGMA journal_mode=WAL; PRAGMA synchronous=NORMAL;');
    sql.exec('CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS withdrawals (id INTEGER PRIMARY KEY, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS kv (k TEXT PRIMARY KEY, v TEXT NOT NULL);');
    ENGINE = 'sqlite';
  } catch (e) { sql = null; console.warn(`⚠ SQLite is not available on this Node version (${e.message}). Falling back to the JSON file. Use Node 22.13+ for SQLite.`); }
}

let db = { users: {}, withdrawals: [], pendingRef: {}, nextWid: 1, settings: {}, daily: {}, secret: '' };
const KV_KEYS = ['settings', 'daily', 'secret', 'nextWid', 'pendingRef'];
const cache = { users: new Map(), wd: new Map(), kv: new Map() };
const clearCache = () => { cache.users.clear(); cache.wd.clear(); cache.kv.clear(); };

(function loadDb() {
  if (ENGINE === 'sqlite') {
    for (const r of sql.prepare('SELECT id, data FROM users').all()) { db.users[r.id] = JSON.parse(r.data); cache.users.set(r.id, r.data); }
    for (const r of sql.prepare('SELECT id, data FROM withdrawals ORDER BY id').all()) { db.withdrawals.push(JSON.parse(r.data)); cache.wd.set(r.id, r.data); }
    for (const r of sql.prepare('SELECT k, v FROM kv').all()) { db[r.k] = JSON.parse(r.v); cache.kv.set(r.k, r.v); }
    if (!Object.keys(db.users).length && fs.existsSync(DB_FILE)) {          // one-time import of an older db.json
      try {
        Object.assign(db, JSON.parse(fs.readFileSync(DB_FILE, 'utf8')));
        fs.renameSync(DB_FILE, DB_FILE + '.migrated');
        console.log(`📦 Imported ${Object.keys(db.users).length} users from db.json into SQLite`);
      } catch (e) { console.error('db.json import failed:', e.message); }
    }
  } else {
...
function persistSql(full) {
  sql.exec('BEGIN');
  try {
    if (full) { sql.exec('DELETE FROM users; DELETE FROM withdrawals; DELETE FROM kv;'); clearCache(); }
    const upU = sql.prepare('INSERT INTO users (id, data) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data');
    for (const id in db.users) { const d = JSON.stringify(db.users[id]); if (cache.users.get(id) !== d) { upU.run(id, d); cache.users.set(id, d); } }
    const upW = sql.prepare('INSERT INTO withdrawals (id, data) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data');
    for (const w of db.withdrawals) { const d = JSON.stringify(w); if (cache.wd.get(w.id) !== d) { upW.run(w.id, d); cache.wd.set(w.id, d); } }
    const upK = sql.prepare('INSERT INTO kv (k, v) VALUES (?, ?) ON CONFLICT(k) DO UPDATE SET v = excluded.v');
    for (const k of KV_KEYS) { const d = JSON.stringify(db[k]); if (cache.kv.get(k) !== d) { upK.run(k, d); cache.kv.set(k, d); } }
    sql.exec('COMMIT');
  } catch (e) { try { sql.exec('ROLLBACK'); } catch (_) {} clearCache(); throw e; }
}
...
function save() {
  ...
  saveTimer = setTimeout(() => {
    saveTimer = null;
    if (ENGINE === 'sqlite') { try { persistSql(false); } catch (e) { console.error('db save error', e.message); } return; }
  ...
  }, ENGINE === 'sqlite' ? 1200 : 400);
}
function flushSync() {
  try { if (ENGINE === 'sqlite') persistSql(false); else fs.writeFileSync(DB_FILE, JSON.stringify(db)); } catch (e) { console.error('flush error', e.message); }
}
process.on('SIGINT', () => { flushSync(); process.exit(0); });
process.on('SIGTERM', () => { flushSync(); process.exit(0); });

/* Backup = all accounts as one JSON document (secret keys are never included). */
const exportDb = () => ({ version: 1, exported_at: Date.now(), users: db.users, withdrawals: db.withdrawals, settings: db.settings, daily: db.daily, nextWid: db.nextWid, pendingRef: db.pendingRef });
function importDb(o) {
  if (!o || typeof o !== 'object' || !o.users || typeof o.users !== 'object' || (o.withdrawals && !Array.isArray(o.withdrawals))) throw new Error('backup file is not valid');
  for (const [id, u] of Object.entries(o.users)) {
    if (!u || String(u.id) !== String(id) || typeof u.balance !== 'number' || !Array.isArray(u.refs)) throw new Error('invalid user record: ' + id);
  }
  db.users = o.users; db.withdrawals = o.withdrawals || []; db.settings = o.settings || {}; db.daily = o.daily || {}; db.pendingRef = o.pendingRef || {};
  db.nextWid = Math.max(Number(o.nextWid) || 1, 1, ...db.withdrawals.map(w => Number(w.id) + 1));
  applySettings();
  if (ENGINE === 'sqlite') persistSql(true); else flushSync();
  return { users: Object.keys(db.users).length, withdrawals: db.withdrawals.length };
}
...
const TG_URL = new URL(env('TG_API_URL', 'https://api.telegram.org'));
const tgHttp = TG_URL.protocol === 'http:' ? http : https;
function tgRequest(pathname, method, headers, body, timeout) {
  return new Promise(resolve => {
    const req = tgHttp.request({ hostname: TG_URL.hostname, port: TG_URL.port || undefined, path: pathname, method, headers, timeout: timeout || 45000 }, res => {
      const chunks = []; res.on('data', c => chunks.push(c));
      res.on('end', () => { const buf = Buffer.concat(chunks); resolve({ status: res.statusCode, buf }); });
    });
    req.on('timeout', () => req.destroy(new Error('timeout')));
    req.on('error', e => resolve({ status: 0, buf: Buffer.from(JSON.stringify({ ok: false, description: e.message })) }));
    req.end(body);
  });
}
async function tgCall(method, payload) {
  if (!CFG.BOT_TOKEN) return { ok: false, description: 'no token' };
  const body = JSON.stringify(payload || {});
  const r = await tgRequest(`/bot${CFG.BOT_TOKEN}/${method}`, 'POST', { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }, body);
  try { return JSON.parse(r.buf.toString()); } catch (_) { return { ok: false, description: 'bad json' }; }
}
async function tgSendDocument(chat_id, filename, buf, caption) {
  if (!CFG.BOT_TOKEN) return { ok: false, description: 'no token' };
  const boundary = '----cfbd' + crypto.randomBytes(8).toString('hex');
  const field = (n, v) => Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="${n}"\r\n\r\n${v}\r\n`);
  const body = Buffer.concat([
    field('chat_id', chat_id), caption ? field('caption', caption) : Buffer.alloc(0),
    Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="document"; filename="${filename}"\r\nContent-Type: application/octet-stream\r\n\r\n`),
    buf, Buffer.from(`\r\n--${boundary}--\r\n`),
  ]);
  const r = await tgRequest(`/bot${CFG.BOT_TOKEN}/sendDocument`, 'POST', { 'Content-Type': 'multipart/form-data; boundary=' + boundary, 'Content-Length': body.length }, body, 120000);
  try { return JSON.parse(r.buf.toString()); } catch (_) { return { ok: false, description: 'bad json' }; }
}
async function tgDownload(filePath) {
  const r = await tgRequest(`/file/bot${CFG.BOT_TOKEN}/${filePath}`, 'GET', {}, undefined, 60000);
...
async function sendBackup(chatIds) {
  const data = exportDb(), gz = zlib.gzipSync(Buffer.from(JSON.stringify(data)));
  const stamp = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-');
  const cap = `💾 ${CFG.APP_NAME} ব্যাকআপ\n👤 ইউজার: ${Object.keys(data.users).length}\n💸 উইথড্র রেকর্ড: ${data.withdrawals.length}\n\nরিস্টোর করতে: এই ফাইলটি বটে পাঠিয়ে ক্যাপশনে লিখুন\n/restore CONFIRM`;
  let sent = 0;
  for (const id of chatIds) {
    const r = await tgSendDocument(id, `cashflowbd-backup-${stamp}.json.gz`, gz, cap);
    if (r.ok) sent++; else console.warn(`⚠ backup to ${id} failed: ${r.description}`);
  }
  return sent;
}
...
async function adminApi(req, res, url, pathname) {
  ...
  const body = req.method === 'POST' ? await readBody(req, route === 'restore' ? 60e6 : 20000) : {};

  if (route === 'stats') return sendJson(res, 200, { ok: true, stats: computeStats(), storage: { engine: ENGINE, file: ENGINE === 'sqlite' ? 'cashflow.sqlite' : 'db.json', users: Object.keys(db.users).length, withdrawals: db.withdrawals.length }, now: Date.now() });

  if (route === 'backup' && req.method === 'GET') return sendJson(res, 200, exportDb());
  if (route === 'backup/send' && req.method === 'POST') {
    if (!CFG.ADMIN_IDS.length) return sendJson(res, 200, { ok: false, error: 'no_admin_ids' });
    return sendJson(res, 200, { ok: true, sent: await sendBackup(CFG.ADMIN_IDS) });
  }
  if (route === 'restore' && req.method === 'POST') {
    if (body.confirm !== 'CONFIRM') return sendJson(res, 400, { ok: false, error: 'confirm' });
    try { return sendJson(res, 200, Object.assign({ ok: true }, importDb(body.backup))); }
    catch (e) { return sendJson(res, 400, { ok: false, error: 'invalid', message: e.message }); }
  }
...
function readBody(req, max) {
  max = max || 20000;
  return new Promise(resolve => {
    let d = ''; req.on('data', c => { d += c; if (d.length > max) req.destroy(); });
...
async function handleRestoreUpload(m) {
  try {
    const f = await tgCall('getFile', { file_id: m.document.file_id });
    if (!f.ok) throw new Error(f.description || 'getFile failed');
    let buf = await tgDownload(f.result.file_path);
    if (buf[0] === 0x1f && buf[1] === 0x8b) buf = zlib.gunzipSync(buf);
    const r = importDb(JSON.parse(buf.toString('utf8')));
    return notify(m.chat.id, `♻️ রিস্টোর সম্পন্ন!\n👤 ইউজার: ${r.users}\n💸 উইথড্র রেকর্ড: ${r.withdrawals}`);
  } catch (e) { return notify(m.chat.id, `❌ রিস্টোর ব্যর্থ: ${esc(e.message)}\nকোনো ডাটা বদলানো হয়নি।`); }
}

async function handleUpdate(up) {
  if (up.message && up.message.document && isAdmin(up.message.from.id) && /^\/restore\s+CONFIRM\b/i.test(up.message.caption || '')) return handleRestoreUpload(up.message);
  if (up.message && up.message.text) {
    ...
    if (isAdmin(from.id)) {
      ...
      if (cmd === '/backup') { await notify(m.chat.id, '💾 ব্যাকআপ তৈরি হচ্ছে…'); const n = await sendBackup([m.chat.id]); return n ? null : notify(m.chat.id, '❌ ব্যাকআপ পাঠানো যায়নি (সার্ভারের লগ দেখুন)।'); }
    }
    return;
  }
...
async function poll() {
  ...
  const hours = num('BACKUP_HOURS', 24);
  if (hours > 0 && CFG.ADMIN_IDS.length) setInterval(() => { sendBackup(CFG.ADMIN_IDS).catch(e => console.warn('auto backup failed', e.message)); }, hours * 3600e3).unref();
  let offset = 0;
  for (;;) {
